...
from util import get_image_resolution, get_image_dimension
...
class RiskMaps():
    ...
    def prepare_data(self):
        ...
        for year in self.years:
            ...
            output_pth = os.path.join(self.drive_folder_path, f'{self.state_name}_{year}.tif')
        ...
        self.gee_manager.euclidean_dist_calc(
            os.path.join(self.drive_folder_path, f'{self.state_name}_{self.start_year}.tif'),
            self.forest_edge_distance_start
        )

        # Calcuate Euclidean distance from forest edge at the start of the confirmation period
        self.gee_manager.euclidean_dist_calc(
            os.path.join(self.drive_folder_path, f'{self.state_name}_{self.mid_year}.tif'),
            self.forest_edge_distance_cnf
        )

        # Calculate Euclidean distance from forest edge at the start of the validity period
        self.gee_manager.euclidean_dist_calc(
            os.path.join(self.drive_folder_path, f'{self.state_name}_{self.end_year}.tif'),
            self.forest_edge_distance_vp
        )

        # Generate deforestation maps
        file1 = os.path.join(self.drive_folder_path, f'{self.state_name}_{self.start_year}.tif')
        file2 = os.path.join(self.drive_folder_path, f'{self.state_name}_{self.mid_year}.tif')
        file3 = os.path.join(self.drive_folder_path, f'{self.state_name}_{self.end_year}.tif')
