...
class GEEManager:
    def __init__(self):
        ...
        self.initialize_gee_and_drive("cogent-range-308518")
    ...
    def create_jurisdiction_mask(self, state_name: str):
        ...
        """
        state = ee.FeatureCollection('FAO/GAUL/2015/level2').filter(ee.Filter.eq('ADM2_NAME', state_name))
    ...
    def compute_distance(self, raster_path, output_path, target_value=0):
        ...
        distance = scipy.ndimage.distance_transform_edt(mask==0)
        # Convert to meters
    ...
    def reproject(self, input_file, output_file):
        with rasterio.open(input_file) as src:
              src_crs = src.crs
              print(src_crs)
              # Get the target CRS as pyproj CRS object
              ...
              # Reproject the raster and save to new file
              with rasterio.open(output_file, 'w', **kwargs) as dst:
                  for i in range(1, src.count + 1):
                      # Read each band
                      band = src.read(i)
                      # Reproject the band to the new CRS
                      reproject(
                          source=band,
              ...
              print(f"Raster successfully reprojected to EPSG:4326 and saved to {output_file}")
